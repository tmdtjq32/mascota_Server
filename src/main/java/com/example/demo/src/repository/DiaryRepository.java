...
public interface DiaryRepository extends JpaRepository<Diary, Integer>, JpaSpecificationExecutor<Diary> {
        ...
        @Query(value = "select distinct diary.* from diary"
                        +
                        " join mood on mood.diary_id = diary.idx" +
                        " join pet on pet.user_id = diary.user_id" +
                        " where mood.type = :type and diary.user_id = :userid and pet.status = 'N'" +
                        " order by diary.idx desc limit 6", nativeQuery = true)
        List<Diary> findByTypeAndUser(@Param(value = "type") String type,
